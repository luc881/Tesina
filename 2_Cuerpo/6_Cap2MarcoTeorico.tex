...
% En este proyecto, el sistema se estructura en tres capas principales: aplicación web, servidor y capa física.

\begin{enumerate}
\item \textbf{Aplicación web (Interfaz de Usuario):}
...
Su función principal es permitir al usuario consultar y registrar información, como ventas, inventario o alertas. La aplicación web no procesa reglas complejas ni almacena datos de forma permanente; solo envía las acciones al servidor y muestra las respuestas recibidas.

\item \textbf{Servidor y API (Lógica del sistema):}  
El servidor constituye el núcleo lógico del sistema. Es responsable de procesar la información, aplicar las reglas del negocio y gestionar el acceso a los datos almacenados en la base de datos.  
...
La comunicación entre la aplicación web y el servidor se realiza mediante una API (Interfaz de Programación de Aplicaciones), que define un conjunto de reglas y formatos estandarizados para el intercambio de información. Gracias a esta API, la aplicación web puede solicitar datos o enviar instrucciones sin conocer los detalles internos del funcionamiento del sistema.
...
Estos dispositivos capturan información del entorno físico y la envían automáticamente al servidor, donde es almacenada y analizada. De esta manera, el sistema integra datos digitales con condiciones reales del entorno, permitiendo una supervisión más precisa y automatizada.

\end{enumerate}

La integración de estas capas permite que el sistema funcione de manera coordinada. Por ejemplo, al escanear un producto con un lector de códigos de barras (Capa Física), el servidor identifica el medicamento correspondiente en la base de datos y envía la información a la aplicación web para que sea mostrada al usuario. De forma similar, si un sensor detecta una variación anormal de temperatura, el servidor puede generar una alerta que se visualiza inmediatamente en la interfaz del administrador. Esta arquitectura modular hace que el sistema sea escalable, confiable y adaptable a las necesidades futuras de la Farmacia Selene.
...
\secbreak
\section{Aplicación Web: Tecnologías}

La aplicación web representa la interfaz gráfica del sistema y constituye el principal medio de interacción entre los usuarios y la plataforma. Su diseño debe garantizar facilidad de uso, rapidez en la respuesta y una presentación clara de la información. 

\subsection{Tecnologías Base: HTML, CSS y JavaScript}

El desarrollo de la interfaz web se apoya en tres tecnologías fundamentales de la web:
...
Es el lenguaje de programación que permite agregar comportamiento dinámico a la interfaz. Gracias a JavaScript, el sistema puede responder a las acciones del usuario, validar datos, actualizar información en pantalla y comunicarse con el servidor sin recargar la página.
...
Para asegurar una apariencia profesional y coherente, la aplicación web emplea \textbf{Material-UI (MUI)}, una biblioteca de componentes prediseñados basada en el estándar de diseño Material Design.
...
En conjunto, estas tecnologías permiten construir una interfaz moderna, eficiente y fácil de usar, que sirve como herramienta principal para la operación diaria de la Farmacia Selene y facilita la transición de procesos manuales a un entorno digital.


\subsection{Vercel: Plataforma de Despliegue de la Aplicación Web}

Para la publicación y disponibilidad en línea de la aplicación web se utiliza \textbf{Vercel}, una plataforma de despliegue en la nube especializada en aplicaciones web modernas.
...
\secbreak
\section{Servidor y API REST}

El servidor es la parte interna del sistema de gestión de inventario y funciona como el centro de control de toda la información. Aunque no es visible para los usuarios, es fundamental para el correcto funcionamiento del sistema, ya que se encarga de procesar los datos, aplicar las reglas del negocio, proteger la información y coordinar la comunicación entre la interfaz de usuario, la base de datos y los dispositivos físicos.

En términos generales, el servidor actúa como un intermediario inteligente: recibe las solicitudes de la aplicación web (por ejemplo, registrar una venta o consultar el inventario), las procesa de acuerdo con las reglas establecidas y devuelve una respuesta con la información correspondiente. Para lograr esto, el servidor se implementa como una \textbf{API (Interfaz de Programación de Aplicaciones)}, es decir, un conjunto de rutas y reglas que permiten el intercambio de información de forma ordenada y segura.

El desarrollo del servidor se realizó utilizando el lenguaje de programación Python y herramientas modernas que facilitan la organización, el mantenimiento y la evolución del sistema.

\subsection{Python}

Python es el lenguaje de programación utilizado para desarrollar el servidor del sistema. Fue elegido por su sintaxis clara y legible, lo que facilita la comprensión del código, la detección de errores y el mantenimiento del sistema, especialmente en proyectos colaborativos. Además, cuenta con un ecosistema amplio y maduro de librerías y frameworks que permiten desarrollar aplicaciones web robustas y escalables de manera eficiente.

En este sistema, Python actúa como la base sobre la cual se implementa la lógica de negocio, gestionando las operaciones de inventario, autenticación de usuarios, registro de datos de sensores y comunicación con la base de datos PostgreSQL, asegurando que el servidor funcione de manera eficiente, confiable y segura.

\subsection{FastAPI: Comunicación entre el Sistema y los Usuarios}

FastAPI es el framework web utilizado para construir el servidor. Un framework puede entenderse como una base de trabajo que proporciona las herramientas necesarias para recibir solicitudes, procesarlas y enviar respuestas.

FastAPI permite definir claramente qué acciones puede realizar el sistema, como consultar productos, registrar ventas o recibir datos de sensores. Cada una de estas acciones se define como un \textbf{endpoint}, que es simplemente una dirección a la que la aplicación web o los dispositivos pueden enviar información.
...
Gracias a FastAPI, el servidor puede comunicarse de manera eficiente tanto con la interfaz web como con los dispositivos físicos del sistema.
...
\subsection{Railway: Plataforma de Despliegue del Servidor}

Para el alojamiento y ejecución del servidor en un entorno accesible desde internet se utiliza \textbf{Railway}, una plataforma de infraestructura en la nube que permite desplegar aplicaciones y bases de datos de manera sencilla.

Railway proporciona un entorno donde el servidor desarrollado con FastAPI puede ejecutarse de forma continua, permitiendo que la aplicación web y los dispositivos físicos se comuniquen con la API desde cualquier ubicación con conexión a internet.
...
Gracias a Railway, el servidor del sistema permanece activo de manera constante, asegurando que las solicitudes provenientes de la aplicación web y de los dispositivos IoT sean atendidas de forma inmediata. Esto permite que el sistema funcione como una plataforma centralizada y accesible, sin necesidad de mantener servidores físicos dentro de la farmacia.
...
\item Transmitir de manera confiable los datos recopilados al servidor usando protocolos de red como HTTP o MQTT.
...
El microcontrolador \textbf{ESP32-C5} es el dispositivo central de la capa física del sistema y actúa como el \textbf{controlador principal} de la red de sensores. Su función es leer la información proveniente de los dispositivos conectados, realizar un procesamiento básico y transmitir los datos al servidor a través de la red inalámbrica.
...
\begin{itemize}
    ...
    \item Enviar los datos recopilados al servidor para su almacenamiento, monitoreo y análisis.
    \item Mantener una conexión estable con la red Wi-Fi de la farmacia utilizando el estándar Wi-Fi 6.
\end{itemize}

El uso del ESP32-C5 permite que la captura de datos ambientales sea automática y continua, sin necesidad de intervención humana, lo que incrementa la confiabilidad del monitoreo y facilita la supervisión continua. Además, su bajo consumo energético y su capacidad de operación en redes densas lo convierten en una solución adecuada para sistemas IoT en entornos comerciales.

\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.7\textwidth]{0_imagenes/Cap2MarcoTeorico/ESP32-C5-DevKitC-1_v1.1_callouts.png}}
    \caption{Microcontrolador ESP32-C5}
    \label{fig:cap2-esp32c5}
...
\subsection{WiFiManager: Gestión Dinámica de Credenciales Wi-Fi}

Durante el despliegue de dispositivos IoT en entornos reales, uno de los desafíos frecuentes es la configuración de la red inalámbrica sin necesidad de recompilar el firmware o acceder físicamente al código fuente. Para resolver esta problemática, el sistema utiliza la librería \textbf{WiFiManager}, desarrollada por tzapu, que permite gestionar las credenciales Wi-Fi del ESP32-C5 de manera dinámica desde cualquier dispositivo con navegador web.

El funcionamiento de WiFiManager se basa en el siguiente mecanismo: si el microcontrolador no encuentra una red Wi-Fi previamente configurada, genera automáticamente un punto de acceso inalámbrico (Access Point) con el nombre \texttt{Sensor-Farmacia}. El usuario puede conectarse a dicho punto desde un teléfono o computadora, acceder a un portal de configuración web y seleccionar la red deseada ingresando sus credenciales. Una vez guardada la información, el dispositivo se reconecta automáticamente a la red configurada cada vez que se reinicia.

Adicionalmente, el sistema incorpora un mecanismo de restablecimiento de credenciales mediante el botón BOOT del ESP32-C5 (GPIO~0). Al mantener presionado dicho botón durante más de tres segundos, el sistema borra las credenciales almacenadas y reinicia el portal de configuración, permitiendo cambiar la red de forma sencilla sin necesidad de herramientas adicionales.

Entre las principales ventajas de utilizar WiFiManager en este proyecto se encuentran:

\begin{itemize}
    \item \textbf{Configuración sin recompilación:} Las credenciales de red se ingresan en tiempo de ejecución, lo que simplifica la instalación del dispositivo en distintos entornos.
    \item \textbf{Portal web integrado:} Ofrece una interfaz gráfica accesible desde cualquier dispositivo con navegador, sin necesidad de instalar software adicional.
    \item \textbf{Persistencia de credenciales:} Los datos de la red se almacenan en memoria no volátil del ESP32-C5, por lo que se mantienen disponibles tras cortes de energía o reinicios.
    \item \textbf{Reconexión automática:} En caso de pérdida de la señal Wi-Fi, el sistema intenta reconectarse de forma autónoma, garantizando la continuidad del monitoreo.
\end{itemize}

El uso de WiFiManager facilita significativamente el proceso de instalación y mantenimiento del nodo sensor en la farmacia, reduciendo la dependencia de personal técnico especializado para tareas de configuración de red.
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.4\textwidth]{0_imagenes/Cap2MarcoTeorico/AR3107-AHT10-Sensor-de-Temperatura-y-Humedad-V2-800x800.jpg}}
    \caption{Sensor AHT10}
    \label{fig:cap2-aht10}
...
Una vez leído el código, la aplicación web solicita al servidor la información asociada al producto, como su nombre, precio y cantidad disponible. Esta información se muestra al usuario de forma automática, haciendo el proceso más rápido y eficiente.
...
\begin{itemize}
    \item \textbf{PyCharm:} Utilizado principalmente para el desarrollo del servidor en Python y la implementación de la lógica del sistema.
    \item \textbf{Visual Studio Code (VS Code):} Empleado en el desarrollo de la interfaz web con tecnologías web como HTML, CSS, JavaScript y React.
    ...
    \item \textbf{Arduino IDE:} Entorno de desarrollo utilizado para programar el microcontrolador ESP32-C5. Permite escribir, compilar y cargar el código directamente en el microcontrolador, e incluye un monitor serie que facilita la depuración del sistema durante su ejecución. Además, incorpora un gestor de librerías desde el cual se instalan las dependencias necesarias para el proyecto, como los controladores del sensor AHT10 y la librería WiFiManager.
\end{itemize}
...
\item Facilita la comunicación segura entre aplicaciones web y servicios del servidor.
\end{itemize}
...
El mantenimiento de la integridad físico-química de los productos farmacéuticos está intrínsecamente ligado al control estricto de las variables macroambientales en sus áreas de almacenamiento y distribución. Los medicamentos catalogados como sensibles a la humedad experimentan tasas de degradación acelerada ante desviaciones térmicas, lo que compromete su eficacia terapéutica y la seguridad del paciente. En el contexto nacional, la Comisión Federal para la Protección contra Riesgos Sanitarios (COFEPRIS) vigila el cumplimiento de un conjunto de Normas Oficiales Mexicanas (NOM) que dictan los criterios de ingeniería y control de calidad aplicables.
...
\begin{itemize}
    \item Mapeo Térmico y de Humedad: Proceso analítico-experimental que exige determinar la distribución tridimensional de las variables ambientales dentro de un volumen de almacenamiento. El sistema automatizado de monitoreo debe desplegar sus sensores (nodos terminales) específicamente en los "puntos críticos" (puntos más fríos, más cálidos y de mayor oscilación de humedad) identificados durante dicho mapeo.


    \item Validación de Sistemas Computarizados (Anexo 6): Cualquier plataforma digital, firmware o software orientado al registro y control de variables que afecten la calidad del medicamento debe ser validado. El sistema de adquisición de datos debe garantizar la integridad, trazabilidad y seguridad de la información, impidiendo la manipulación o pérdida de registros históricos (principios ALCOA+).
\end{itemize}

\subsection{NOM-073-SSA1-2015: Estudios de Estabilidad de Fármacos y Medicamentos}

La NOM-073-SSA1-2015 define los lineamientos para evaluar la estabilidad de los compuestos bajo diferentes zonas climáticas mundiales. Para México (clasificado predominantemente en las Zonas Climáticas II y IVb), los estudios de estabilidad a largo plazo fijan condiciones estándar que el sistema de gestión debe supervisar de forma continua:
\begin{itemize}
    \item Condición General de Almacenamiento (Temperatura Ambiente): $30 \text{ }^{\circ}\text{C} \pm 2 \text{ }^{\circ}\text{C}$ con una Humedad Relativa (HR) de $65\% \pm 5\%$.
    \item Condición de Red de Frío (Refrigeración): $5 \text{ }^{\circ}\text{C} \pm 3 \text{ }^{\circ}\text{C}$ (rango operativo estricto de $2 \text{ }^{\circ}\text{C}$ a $8 \text{ }^{\circ}\text{C}$).
\end{itemize}
