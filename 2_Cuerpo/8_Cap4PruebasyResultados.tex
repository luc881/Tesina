...
La \textbf{integridad relacional} es la propiedad que garantiza que los datos almacenados en las distintas tablas de la base de datos sean coherentes entre sí: que una venta siempre apunte a un producto que existe, que no haya dos usuarios con el mismo correo, y que los campos obligatorios nunca queden vacíos. Esta coherencia no depende de que el programador la vigile manualmente; se implementa mediante \textbf{restricciones}, reglas declaradas en el propio esquema que el motor de base de datos aplica de forma automática y rechaza cualquier operación que las incumpla. Las pruebas de esta sección verifican precisamente que esas reglas actúan como se espera.

Las restricciones definidas en el esquema (claves foráneas, unicidad y valores no nulos) se verificaron a través de los casos de prueba negativos de la API: intentar crear un lote con un producto inexistente devuelve error, igual que registrar un usuario con un correo ya ocupado o completar una venta vacía. En todos los casos el motor de base de datos aplicó la restricción correspondiente y la API tradujo ese rechazo en un código de respuesta apropiado.

Un caso de diseño particular que se verificó fue el de los lotes sin número de código asignado. El inventario acepta lotes sin código (ocurre cuando el fabricante no imprime uno), pero si el código está presente no puede repetirse para el mismo producto. Para implementar esto sin bloquear los lotes sin código se utilizó un índice parcial: la restricción de unicidad aplica únicamente cuando el campo tiene un valor, ignorando los registros en que está vacío. La Fig.~\ref{fig:cap4-lote-index} ilustra el comportamiento verificado con cinco intentos de registro sobre un mismo producto: los dos primeros lotes, con códigos distintos, son aceptados; el tercero es rechazado por repetir un código ya registrado; y los dos últimos, ambos sin código, son aceptados sin conflicto porque los registros vacíos no compiten entre sí.
...
\caption{Comportamiento del índice parcial sobre el código de lote}
\label{fig:cap4-lote-index}
\vspace{0.2cm}
\footnotesize \textbf{Fuente:} Elaboración propia.
\end{figure}

\subsection{Evolución del esquema durante el desarrollo}
...
Para aceptar lotes sin código asignado (situación habitual con algunos proveedores) sin perder la garantía de que los códigos presentes no se dupliquen, se reemplazó la restricción de unicidad simple por un índice que aplica únicamente cuando el campo tiene valor. \\
\hline
...
Se identificó durante las pruebas que las consultas que combinan tablas, por ejemplo obtener un lote junto con su producto, resultaban lentas sin índices en las columnas de referencia. Se añadieron índices en las claves foráneas de las tablas de mayor uso. \\
\hline
...
El esquema final de la base de datos opera establemente en el entorno de producción de Railway. Todas las restricciones verificadas (integridad referencial, unicidad condicional, borrado lógico y precisión monetaria) funcionan correctamente bajo la carga de las pruebas automatizadas. Los seis ajustes documentados en la tabla anterior resolvieron los comportamientos incorrectos que aparecieron durante el desarrollo, y ninguno requirió modificaciones posteriores.
...
Para verificar el funcionamiento de la API se probó cada uno de sus módulos contra la base de datos real del sistema, en lugar de datos simulados. La razón es simple: una verificación sobre datos simulados puede dar resultados correctos aunque el sistema real falle, mientras que una prueba sobre la base de datos real detecta ese tipo de diferencias, de modo que lo verificado es idéntico a lo que se ejecuta en producción. Se cubrieron escenarios positivos (operaciones que deben completarse con éxito) y negativos (operaciones que el sistema debe rechazar) en los módulos de autenticación, usuarios, roles, productos, inventario, ventas, compras, proveedores y sensor.
...
Para resolver este comportamiento fue necesario incorporar un segundo token de mayor duración, denominado token de refresco. El token de acceso mantiene su vigencia de 30 minutos para las operaciones regulares; cuando está próximo a expirar, el sistema lo renueva sin que el usuario tenga que iniciar sesión otra vez. Se añadió además la opción de sesión prolongada, equivalente a la función \textit{recordarme} de otras aplicaciones web, que extiende la duración del token de refresco a 30 días para quien la seleccione al iniciar sesión. La Fig.~\ref{fig:cap4-token-timeline} compara ambas versiones sobre una línea de tiempo. En la fila superior (\textit{Antes}) el único token expira a los 30 minutos y la sesión se cierra. En la fila inferior (\textit{Después}), cada vez que el token está por expirar se genera uno nuevo en silencio, encadenando periodos de 30 minutos hasta por 30 días sin que el usuario perciba la renovación.
...
\begin{figure}[H]
   \centering
   \fbox{\includegraphics[width=0.65\textwidth]{0_imagenes/Cap4/LoginRecordarme.png}}
...
\caption{Evolución del sistema de sesión}
...
La lógica de completar una venta, que descuenta las unidades vendidas del inventario y registra de qué lote provienen, fue el componente más complejo de la API. Con la propuesta inicial del registro de ventas se contaba únicamente con el encabezado de la transacción y el detalle de los productos; el descuento efectivo del inventario por lote no estaba implementado en esa versión.
...
La Fig.~\ref{fig:cap4-fefo} ilustra el segundo caso con un escenario verificado en las pruebas: una venta de 150 unidades con tres lotes disponibles. El sistema consume primero el lote A por ser el de caducidad más próxima y lo agota con sus 80 unidades; las 70 restantes se descuentan del lote B, que es el siguiente en vencer; el lote C, con la caducidad más lejana, no se toca. El total descontado coincide exactamente con la cantidad vendida.
...
\caption{Ejemplo de la política FEFO aplicada en el módulo de ventas}
\label{fig:cap4-fefo}
\vspace{0.2cm}
\footnotesize \textbf{Fuente:} Elaboración propia.
\end{figure}
...
\subsection{Ajustes realizados a la API}
...
Con los ajustes realizados, la API quedó desplegada de forma permanente en la plataforma Railway. Desde ahí, el servicio se mantiene en operación continua y recibe las solicitudes de la interfaz web y del nodo sensor. La plataforma registra cada actualización de código como un nuevo despliegue y muestra el estado del servicio en tiempo real, lo que permite verificar que la aplicación esté activa sin necesidad de acceder al servidor directamente.

La figura~\ref{fig:cap4-railway} muestra el panel de Railway con el servicio de la API en estado activo.

% TODO: insertar captura de pantalla del dashboard de Railway mostrando el servicio activo
% Guardar en: 0_imagenes/Cap4/RailwayDashboard.png
\begin{figure}[H]
   \centering
   \fbox{\includegraphics[width=0.55\textwidth]{0_imagenes/Cap4/RailwayDashboard.png}}
   \caption{Panel de Railway con el servicio de la API en estado activo}
   \label{fig:cap4-railway}
   \vspace{0.2cm}
   \footnotesize \textbf{Fuente:} Elaboración propia.
\end{figure}
...
La verificación de la interfaz web se realizó de forma iterativa a lo largo del desarrollo: cada módulo se probó manualmente en el navegador al momento de construirse, conectándolo contra la API desplegada en Railway para verificar que los datos fluyeran correctamente de extremo a extremo.

La Fig.~\ref{fig:cap4-wireflow} muestra el flujo general de navegación verificado en las pruebas. La fila superior es el recorrido principal de la operación diaria: del inicio de sesión al Dashboard y de ahí al registro y confirmación de una venta. Los módulos de la parte inferior, conectados con flechas punteadas, son accesibles en cualquier momento desde el menú lateral de la aplicación.
...
% 1. Login
% TODO: reemplazar parbox por \includegraphics[width=3.8cm,height=2.4cm]{0_imagenes/Cap4/Login.png}
...
% TODO: reemplazar parbox por \includegraphics[width=4.0cm,height=2.4cm]{0_imagenes/Cap4/Dashboard.png}
...
% 3. Nueva venta
% TODO: reemplazar parbox por \includegraphics[width=3.8cm,height=2.4cm]{0_imagenes/Cap4/NuevaVenta.png}
...
% 5. Usuarios y roles
% TODO: reemplazar parbox por \includegraphics[width=3.8cm,height=2.4cm]{0_imagenes/Cap4/Usuarios.png}
...
% 6. Inventario
% TODO: reemplazar parbox por \includegraphics[width=3.8cm,height=2.4cm]{0_imagenes/Cap4/Inventario.png}
...
% 7. Compras
% TODO: reemplazar parbox por \includegraphics[width=3.8cm,height=2.4cm]{0_imagenes/Cap4/Compras.png}
...
% 8. Sensor ambiental
% TODO: reemplazar parbox por \includegraphics[width=3.8cm,height=2.4cm]{0_imagenes/Cap4/Sensor.png}
...
\caption{Flujo de navegación del usuario en la aplicación web}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=\textwidth]{0_imagenes/Cap4/Dashboard.png}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.8\textwidth]{0_imagenes/Cap4/ModoLector.png}}
    \caption{Formulario de registro de venta en modo lector}
...
Con los ajustes realizados, la interfaz web quedó desplegada de forma permanente en la plataforma Vercel. Cada actualización de código genera un nuevo despliegue automático, y la plataforma registra el historial de versiones junto con el estado de cada una. La figura~\ref{fig:cap4-vercel} muestra el panel de Vercel con el proyecto activo y el último despliegue exitoso.

\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.95\textwidth]{0_imagenes/Cap4/VercelDashboard.png}}
    \caption{Panel de Vercel con el proyecto de la interfaz web en estado activo}
    \label{fig:cap4-vercel}
    \vspace{0.2cm}
    \footnotesize \textbf{Fuente:} Elaboración propia.
\end{figure}
...
La investigación del problema reveló que el pin del microcontrolador asignado al LED RGB coincidía con el mismo pin utilizado como línea de datos del bus de comunicación con el sensor AHT10. Al intentar controlar el LED, el firmware generaba señales que interferían con la comunicación del sensor, corrompiendo las lecturas. La Fig.~\ref{fig:cap4-pin-conflicto} ilustra el conflicto: del pin GPIO~8 del microcontrolador se desprenden dos destinos simultáneos, la línea de datos del sensor y el control del LED integrado en la placa. Toda señal dirigida al LED viajaba también por la línea del sensor, y esa interferencia era lo que corrompía las lecturas.
...
\caption{Conflicto de pines detectado durante las pruebas}
...
La Fig.~\ref{fig:cap4-rangos} compara sobre una misma escala los dos niveles de control que resultan de este diseño. La franja clara es el rango de validación del firmware, es decir, lo que el nodo acepta como lectura físicamente posible; la franja oscura es el rango que la NOM-073-SSA1-2015 recomienda para almacenar medicamentos. Una lectura fuera de la franja clara se descarta por ser un error del sensor; una lectura dentro de la franja clara pero fuera de la oscura sí se registra y genera una alerta en la aplicación, porque describe una condición real que requiere atención.
...
\caption{Rangos de validación del firmware frente a los rangos de la NOM-073-SSA1-2015}
...
\clearpage
\section{Pruebas del gabinete impreso en 3D}
...
\caption{Vista lateral esquemática del problema del gabinete original}
\label{fig:cap4-gabinete-problema}
\vspace{0.2cm}
\footnotesize \textbf{Fuente:} Elaboración propia.
\end{figure}

Para resolver esto fue necesario medir manualmente el conjunto ya ensamblado, incluyendo los conectores y el radio de curvatura de los cables, y rediseñar el gabinete con esas dimensiones reales. La nueva versión incorpora una altura interior mayor en la base para dar espacio a los conectores y una distribución diferente de los componentes para reducir el recorrido de los cables internos.

Con el segundo diseño fabricado se montaron todos los componentes y se incorporaron al firmware las tres pantallas OLED. Como los tres modelos de pantalla son idénticos y utilizan la misma dirección de comunicación, no es posible conectarlas al mismo canal sin que colisionen entre sí: el microcontrolador no podría distinguir a cuál le está enviando instrucciones. La solución fue asignar a cada pantalla su propio par de pines, de modo que opera en un canal de comunicación completamente independiente. La pantalla de temperatura muestra el valor actual en caracteres grandes; la de humedad hace lo mismo con la lectura de humedad relativa; la tercera indica el estado de la conexión WiFi y de la comunicación con la API. Al encender el nodo, las tres pantallas muestran el nombre del sistema de forma simultánea hasta que la primera lectura esté disponible.

\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.75\textwidth]{0_imagenes/Cap4/GabineteFinal.jpeg}}
    \caption{Nodo sensor ensamblado en el gabinete final}
    \label{fig:cap4-gabinete-final}
    \vspace{0.2cm}
    \footnotesize \textbf{Fuente:} Elaboración propia.
\end{figure}

\clearpage
\section{Pruebas del lector de código de barras}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.70\textwidth]{0_imagenes/Cap4/LectorEnUso.jpeg}}
...
% =============================================
% SECCIÓN 4.7: INTEGRACIÓN DEL SISTEMA
% =============================================

\clearpage
\section{Prueba de integración del sistema}

Una vez verificado cada componente por separado, se realizó una prueba de integración del sistema completo para confirmar que los cinco objetivos específicos operan de forma conjunta. El escenario de prueba cubrió el flujo de extremo a extremo más representativo del sistema: desde la medición ambiental del nodo sensor hasta el registro de una venta con descuento de inventario, pasando por todas las capas del sistema. La Fig.~\ref{fig:cap4-integracion} resume el recorrido verificado: el nodo sensor envía sus lecturas a la API, que las almacena en la base de datos; la interfaz web consulta esa misma API tanto para mostrar las lecturas en el tablero como para registrar las ventas, de modo que todos los componentes convergen en un único punto de intercambio de información.
...
\caption{Flujo de integración del sistema completo}
...
El único elemento pendiente al momento de la redacción de este capítulo son las pruebas de operación continua del sistema en las instalaciones de la farmacia, previstas para realizarse una vez que el gabinete físico sea entregado e instalado en el local.
