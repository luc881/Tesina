...
Para verificar el funcionamiento de la API se probó cada uno de sus módulos contra la base de datos real del sistema, en lugar de datos simulados. Este enfoque garantiza que el comportamiento verificado sea idéntico al que se ejecuta en producción: una verificación sobre datos simulados puede dar resultados correctos aunque el sistema real falle, mientras que una prueba sobre la base de datos real detecta ese tipo de diferencias. Se cubrieron escenarios positivos —operaciones que deben completarse con éxito— y negativos —operaciones que el sistema debe rechazar—, en los módulos de autenticación, usuarios, roles, productos, inventario, ventas, compras, proveedores y sensor.
...
Para resolver esto fue necesario relevar manualmente las dimensiones reales del conjunto ensamblado —incluyendo los conectores y el radio de curvatura de los cables— y rediseñar el gabinete con esas medidas. La nueva versión incorpora una altura interior mayor en la base para dar espacio a los conectores y una distribución diferente de los componentes para reducir el recorrido de los cables internos.
...
\begin{figure}[H]
    ...
    \caption{Nodo sensor ensamblado en el gabinete final: ESP32-C5, sensor AHT10 y las tres pantallas OLED en operación}
